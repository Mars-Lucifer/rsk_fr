...
// Карта в подставке показывается уменьшенной. Это витрина разделов, а не сама карта
// набора: её берут отсюда в разбор, где она и приходит к натуральному размеру.
//
// Число подобрано по кадру: без уменьшения двенадцать карт и подошедший вплотную экран
// в один кадр не помещаются — ряд из шести занимал 0.67 м при кадре шириной около 0.6 м.
// На 0.5 ряд укладывается в 0.33 м и целиком встаёт справа от экрана.
//
// Перелёт в разбор при этом читается как увеличение: карта выходит из витрины и приходит
// к размеру, на котором её разбирают по полям.
const SHOW = 0.5;
const W = (CARD_MM.w / 1000) * SHOW;
const H = (CARD_MM.h / 1000) * SHOW;
const T = CARD_MM.thickness / 1000;
const CORNER = (CARD_MM.corner / 1000) * SHOW;
