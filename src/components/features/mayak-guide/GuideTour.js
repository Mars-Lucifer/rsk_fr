...
export default function GuideTour({ steps, open, onClose, onFinish, title = "Инструкция" }) {
    ...
    //
    // Листается всё, включая шаги, которые ждут действия. Раньше стрелка вперёд на таких
    // шагах молчала — чтобы инструкция не превращалась в слайды. На деле выходило хуже:
    // человек жмёт стрелку, ничего не происходит, и вывод один — «клавиши не работают».
    // Кто хочет сделать — сделает и без запрета, шаг закроется сам по факту действия.
    useEffect(() => {
        if (!open) return undefined;
        const onKey = (event) => {
            if (event.key === "Escape") {
