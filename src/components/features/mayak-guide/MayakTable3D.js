...
const PROMPT_CARD_AT = [-0.505, 0.191, -0.674];
...
const PROMPT_VIEW = {
    eye: [-0.716, 0.441, -0.07],
    look: [-0.711, 0.115, -0.655],
};

// Зона разбора идёт в две ступени, и это не украшение, а следствие геометрии стола.
//
// Ноутбук стоит в дальнем левом углу — на месте прежнего планшета. Кадр, в котором его
// экран читается, смотрит из угла в угол, и справа от ноутбука в нём оказывается пустая
// дальняя часть стола: ряд подставок туда не поставить, он вылезет за кромку. Кадр же,
// который держит весь ряд, отодвигает экран настолько, что форма перестаёт читаться.
//
// Поэтому сначала показывается ряд разделов — тот же общий вид зоны, что был раньше, —
// и только когда карта выбрана, камера садится за ноутбук. Это и есть жест участника:
// взял карту из подставки и сел с ней за стол.
const PROMPT_PICK_VIEW = {
    eye: [-0.3, 0.5, 0.59],
    look: [-0.3, 0.1, -0.31],
};
...
export default function MayakTable3D() {
    ...
    const view =
        role && spot?.id === "roles"
            ...
            : // Пока раздел не выбран, зона разбора показывает ряд подставок; выбранная
            // карта пересаживает камеру за ноутбук — см. PROMPT_PICK_VIEW.
