...
// Описание фазы — единственное место, где рассказан ход этапа. Раньше то же самое
// дублировалось списком «Ход этапа» под плеером; список убран, содержание влито сюда.
const PHASES = [
    {
        id: 0,
        label: "Раскладка",
        text: "Стопка «Старт» ложится в верхний правый угол поля, шесть колод по типам контента — на свои гексы. Миплы встают на центральный гекс СТАРТ.",
    },
    {
        id: 1,
        label: "Внутренний круг",
        text: "Четыре ячейки вокруг центра. Команда идёт по часовой стрелке — вправо, вниз, влево, вверх — и решает стартовые задания вместе. Из верхней ячейки выходит на кольцо типов.",
        more: "Раздел «Старт» проходится целиком, по порядку и без пропусков: распределение ролей, входной бланк, ранжирование промптов, затем задания на составление промпта по методологии МАЯК-ОКО.",
    },
    {
        id: 2,
        label: "Внешний круг",
        text: "Команда обходит шесть ячеек кольца — по одной на каждый тип контента. На каждой берёт первую карту типа: это задание 1 из 4.",
        more: "Первая карта помечена цифрой 1 на рубашке и кладётся сверху своей стопки. Пройдя все шесть типов, команда видит весь спектр возможностей ИИ.",
    },
    {
        id: 3,
        label: "Специализация",
        text: "Участники расходятся по своим типам. Каждый идёт по лучу наружу: ещё три задания, три гекса — задания 2, 3 и 4.",
        more: "Направление участник выбирает сам и дальше работает индивидуально. Мипл двигается по лучу: одно выполненное задание — один гекс.",
    },
    {
        id: 4,
        label: "Итог",
        text: "Первая карта — на одиночном гексе внешнего круга, ещё три — по лучу. Итого четыре задания. Участник приносит команде красную Звезду-Джокер.",
        more: "Этап заканчивается, когда в одной из команд все участники выполнили по 4 задания своей специализации; возможен вариант с ограничением по таймеру. Звезда-Джокер понадобится на этапе «МЫ».",
    },
];
    ...
    return (
        <div className={`player ${bare ? "bare" : ""}`}>
            ...
            <style jsx>{`
                ...
                /* Подробность показывается только у активной фазы: пять развёрнутых
                   описаний сразу превращают колонку в стену текста. */
                .phase .more {
                    display: block;
                    margin-top: 8px;
                    padding-top: 8px;
