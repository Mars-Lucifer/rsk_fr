...
import { normalizeQwenTokenEntries } from "../../../../lib/mayakQwen.js";
...
function normalizeImportedTokens(payload) {
    return normalizeQwenTokenEntries(extractTokenSource(payload)).map((entry, index) => {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (importedTokens.length === 0) {
            return res.status(400).json({ success: false, error: "В файле не найдены Qwen-токены" });
        }

        const settings = await readMayakSettings();
        settings.qwenTokens = importedTokens;
        ...
        console.error("[QwenTokens] import failed:", error);
        return res.status(500).json({
            success: false,
            error: error.message || "Не удалось импортировать Qwen-токены",
        });
    }
}
